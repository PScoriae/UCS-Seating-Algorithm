class Person:
    def __init__(self, name=None, prevNeighbour=None, summedComfortVal=None):
        self.name = name
        self.prevNeighbour = prevNeighbour
...
def expandAndReturnChildren(pairComfort, currentPerson):
    ...
    for [m, n, c] in pairComfort:
        if m == currentPerson.name:
            children.append(Person(n, currentPerson.name, currentPerson.summedComfortVal + c))
        elif n == currentPerson.name:
            children.append(Person(m, currentPerson.name, currentPerson.summedComfortVal + c))
    return children
...
def ucs(pairComfort, initialPerson, numOfPersons):
    ...
    while True:
        ...
        print("")
    print(seated[-1].summedComfortVal + getCost(pairComfort, initialPerson, seated[-1].name))
